...
drinks %>% 
  pivot_longer(soda:sparkling_water,
              names_to='drink_type',
...
# to make longer- take measure columns and create value pairs
gap %>% 
  pivot_longer(
    lifeExp:gdpPercap, 
    names_to = 'measure',
...
#as a label for the column, it creates a key-value pair
gap_wide <- gap %>% 
...
#be able to extract year from making the wide data long
...
#make bob ross data tidier then
...
bob %>% 
  ...
  filter((str_detect(item, 'fname') & is_present == 1) |
